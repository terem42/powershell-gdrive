...
function post-urlencoded-req($requestUrl, $params_hash) {
  ...
  try { 
    ...
    Write-Host " success token request response = $($respTxt)"
   ...
   { 
            ...
            Write-Host "exception fill name= $($_.Exception.GetType().fullname)"  
   ...
   finally { if ($null -ne $stream -and $stream -is [System.IDisposable]) { $stream.Dispose() } }
   ...
   while (($bytesRead = $br.Read($buffer, 0, $buffer.Length)) -gt 0)
   {
      if ($first -And !$resumable_location) {
         ...
         if ($file_descriptor.length -lt $BlockSize) {
            Write-Host "file size less than our block size, send as multipart stream in one go"
            $repeat_op = $False
            $req_loop_attempts = 0
            do {
...
function sync-local-folder-to-remote-folder($local_folder,$remote_folder) {
 ...
 Write-Host "filelist to upload = $flist_to_upload"      
 if ($flist_to_upload.Count) {
    $flist_to_upload | % {
